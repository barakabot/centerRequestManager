...
// Simple SHA-256 hash function (same as auth.ts)
...
async function main() {
  ...
  const planningHash = await hashPassword('planning123')
  const tehranHash = await hashPassword('tehran123')
  const isfahanHash = await hashPassword('isfahan123')
  const shirazHash = await hashPassword('shiraz123')
  const mashhadHash = await hashPassword('mashhad123')
  const tabrizHash = await hashPassword('tabriz123')

  await db.user.create({
    data: { name: 'مدیر سیستم', email: 'admin@target.sys', password: adminHash, role: 'admin', branchId: null, isActive: true },
  })
  await db.user.create({
    data: { name: 'برنامه‌ریز فروش', email: 'planning@target.sys', password: planningHash, role: 'planning', branchId: null, isActive: true },
  })
  await db.user.create({
    data: { name: 'مدیر شعبه تهران', email: 'tehran@target.sys', password: tehranHash, role: 'branch_manager', branchId: branchTehran.id, isActive: true },
  })
  await db.user.create({
    data: { name: 'مدیر شعبه اصفهان', email: 'isfahan@target.sys', password: isfahanHash, role: 'branch_manager', branchId: branchIsfahan.id, isActive: true },
  })
  await db.user.create({
    data: { name: 'مدیر شعبه شیراز', email: 'shiraz@target.sys', password: shirazHash, role: 'branch_manager', branchId: branchShiraz.id, isActive: true },
  })
  await db.user.create({
    data: { name: 'مدیر شعبه مشهد', email: 'mashhad@target.sys', password: mashhadHash, role: 'branch_manager', branchId: branchMashhad.id, isActive: true },
  })
  await db.user.create({
    data: { name: 'مدیر شعبه تبریز', email: 'tabriz@target.sys', password: tabrizHash, role: 'branch_manager', branchId: branchTabriz.id, isActive: true },
  })
  ...
  console.log('  Admin:      admin@target.sys / admin123')
  console.log('  Planning:   planning@target.sys / planning123')
  console.log('  Tehran BM:  tehran@target.sys / tehran123')
  console.log('  Isfahan BM: isfahan@target.sys / isfahan123')
  console.log('  Shiraz BM:  shiraz@target.sys / shiraz123')
  console.log('  Mashhad BM: mashhad@target.sys / mashhad123')
  console.log('  Tabriz BM:  tabriz@target.sys / tabriz123')
}
